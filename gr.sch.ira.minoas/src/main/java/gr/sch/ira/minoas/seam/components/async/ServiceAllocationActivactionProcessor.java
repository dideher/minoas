...
public class ServiceAllocationActivactionProcessor extends BaseDatabaseAwareSeamComponent {
    ...
    public Collection<ServiceAllocation> getServiceAllocationThatShouldBeActivated(EntityManager em, Date today) {
        return em.createQuery("SELECT s from ServiceAllocation s WHERE s.employee.active IS TRUE AND s.active IS FALSE AND (s.deleted IS FALSE OR s.deleted IS NULL) AND (s.autoCanceled IS FALSE OR s.autoCanceled IS NULL) AND :onDate  BETWEEN s.established AND s.dueTo ORDER BY s.established").setParameter("onDate", today).getResultList();
    }
	...
	public QuartzTriggerHandle scheduleServiceAllocationActivation(@Expiration Date when, 
	    ...
	    Date today = new Date();
	    info("will check for service allocations that should be set activated on #0", today);
	    ...
	    info("found totally #0 activated that should be activated", serviceAllocationsThatShouldBeActivated.size());
