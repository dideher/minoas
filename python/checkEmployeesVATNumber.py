...
def db_lookup(cursor):
	cursor.execute('SELECT e.ID, e.LAST_NAME, e.FIRST_NAME, e.FATHER_NAME, s.SPECIALIZATION_ID, s.TITLE, ei.REGISTRY_ID, e.VAT_NUMBER, e.EMPLOYEE_TYPE FROM EMPLOYEE e'+
   ...
   ' WHERE e.IS_ACTIVE=1 ORDER BY e.LAST_NAME, e.FIRST_NAME')

def checkEmployeeVAT(employeeVAT):
	# we should add the http://stackoverflow.com/questions/4375511/greek-vat-validation-number-code validation code
	if employeeVAT and len(employeeVAT)==9:
		return True
	else:
		return False
...
def main(argv):
	...
	bad_registryID_table = PrettyTable(["#", "ID", "Surname", "Name", "Father Name", "Specialization", "Type", "VAT Nr"])

	bad_registryID_count = 1; # counter 
	for row in cursor:
		# check employee registry id
		if not checkEmployeeVAT(row['VAT_NUMBER']):
			bad_registryID_table.add_row([bad_registryID_count, row['ID'], row['LAST_NAME'], row['FIRST_NAME'], row['FATHER_NAME'], row['SPECIALIZATION_ID'], row['EMPLOYEE_TYPE'], row['VAT_NUMBER']]);
			bad_registryID_count+=1
	
	if generateHtml:
		print "<h1>Employees with bad or not existent VAT Number</h1>"
	else:
		print "********************************************************"
		print "* Employees with bad or not existent VAT Number"
		print "********************************************************"
