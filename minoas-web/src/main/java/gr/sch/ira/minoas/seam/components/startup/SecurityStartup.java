...
public class SecurityStartup extends BaseDatabaseAwareSeamComponent {
	...
	protected Principal createPrincipal(String username, String password, String realname) {
		...
		if (principal == null) {
			...
			info("principal \"%s\" already exists in the database", username);
		}
		return principal;
	}
	...
	protected Role createRole(String name, String desc, Principal creator) {
		...
		if (role == null) {
			...
			info("successfully created role \"%s\" with description \"%s\" in database.", role.getName(), role
					.getDescription());
		} else {
			info("role \"%s\" already exists in the database.", name);
		}
		return role;
	}
