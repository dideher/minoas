...
public class SecondmentAwareUnitSuggester extends BaseDatabaseAwareSeamComponent {
	...
	public Collection<Unit> suggest(Object secondmemt_search_pattern) {
		...
		return getEntityManager().createQuery("SELECT u FROM Unit u WHERE LOWER(u.title) LIKE LOWER(:search_pattern)").setParameter("search_pattern", CoreUtils.getSearchPattern(String.valueOf(secondmemt_search_pattern))).getResultList();
	}
